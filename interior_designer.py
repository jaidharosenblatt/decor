    ...
    def log_usage_info(self, response, variation_num: int):
        ...
        try:
            if hasattr(response, "usage_metadata"):
                ...
                if has_image_output:
                    print("   (Image output: 1290 tokens flat rate)")

        except Exception as e:
            print(f"Could not log usage info: {e}")

    async def generate_variations(
        self, images: List[str], prompts: list, output_dir: str, num_variations: int = 3
...
{prompt}"""
        ...
        for result in results:
            if isinstance(result, Exception):
                ...
                if image:
                    ...
                    # Save each variation
                    output_path = (
                        f"{output_dir_for_saving}/design_variation_{i + 1:02d}.png"
                    )
        ...
        if total_tokens > 0:
            ...
            print(
                f"   Total output tokens: {len(generated_images) * 1290:,} (1290 per image)"
            )
